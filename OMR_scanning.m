...
I = rgb2gray(imread('omr_test_01.png')); %path of the saved OMR sheet image
str='ABCDE';

%options marked in the sheet
str2='';
startx = 363;
starty = 252; %coordinates of the left most corner of the first choice box
%These coordinates can be found out by using ginput function in MATLAB

diffx = 32;
diffy = 27; %approximate size of the rectangular choice boxes
...
for i = 1:25 %this loop runs for each of the 25 rows of boxes on the left side of the OMR sheet
    ...
    for j = 1:5 %this loop runs for each of the 5 choice boxes in each row
        sy = starty+(j-1)*(diffy+4); % estimating the starting ycoordinate for each choice box
        
        % So, when i=3 and j=4, we are referring to the choice box D of problem 3 of Section-I
        % and (sx,sy) represent the estimated starting coordinate pair for the corresponding box.
        ...
        % So, if we calculate the total number of gray pixels in this rectangular box, the sum will be
        % nearly same as that for our choice box.
        
        n=0;
        for k=1:diffy %these two loops pick each pixel in our rectangular box, k corresponds to y
            for l=1:diffx %coordinate and l corresponds to xcoordinate of the pixel
                a(k,l)=I(sx+l,sy+k);
                if(a(k,l)<=t)
        ...
        mark(sx,sy,1)=255;
        mark(sx,sy,2)=0;
        mark(sx,sy,3)=0; %make (sx,sy) red
    end
    if(f==(6*s))
        startx = startx-adjustx;
        s = s+1; %adjustment done after every 6 rows to reduce estimation error
    end
    f=f+1;
end
...
answer = {'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C';'C'};
...
imshow(c)
str2
fprintf('marks scored:');
marks= marks*4-g;
disp(marks);
fprintf('correct answers:');
disp(k);
fprintf('incorrect answers:');
disp(g);
fprintf('not attempted:');
disp(l);